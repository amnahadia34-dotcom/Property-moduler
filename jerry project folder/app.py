...
if uploaded_files:
    ...
    # ---------- Comprehensive View ----------
    if property_dfs:
        st.markdown("### 📈 All Properties - Comprehensive View")
        combined = pd.concat(property_dfs, axis=0, ignore_index=True)
        combined = combined.head(200)
        st.markdown(
            f"<div class='banner-top'>Displaying {len(combined)} fields across {len(property_dfs)} property files</div>",
            unsafe_allow_html=True)
        st.dataframe(combined,use_container_width=True,hide_index=True)

    st.markdown(
        "<div class='footer'>✅ Backend connected | Synced successfully</div>",
        unsafe_allow_html=True)
